        ...
        while True:
            try:
                ...
                if len(data) == 0:
                    ...
                    if data.startswith("@add") or data.startswith("@edit"):
                        components = data.split(" ")
                        if len(components) < 3:
                            self.envia("Comando no válido: faltan parámetros", 1)
                            continue
    ...
    def exec_command(self, command):
        if command.startswith("@p2pR"):
            ...
            if command[1].startswith("@list"):
                self.envia(f"@p2pA @list {res}", 0)
            else:
                self.envia("@p2pA {}".format(res), 0)
        elif command.startswith("@p2pA"):
            res = command.split(" ", 1)
            if res[1].startswith("@list"):
                part1 = res[1].split(" ", 1)[1]
                all_words = part1 + self.dao.exec_command("@list")
                self.envia(all_words, 1)
            else:
                self.envia(res[1], 1)
        else:
            if command.startswith("@list"):
                self.envia(f"@p2pR {command}", 0)
            else:
